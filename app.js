(() => {
  ...
  function renderUnifiedEditors(focus=null){
    ...
    if(focus){const editorKey=focus.editorId||'primary',target=$(`[data-editor-id="${editorKey}"] [data-block-id="${focus.blockId}"] .block-content`),host=target?.closest('.block-list');host?.focus();if(target){placeCaretAtEnd(target);if(focus.start)getSelection()?.collapse(target,0);}}
  }
  ...
  function bindUnifiedEditor(root,record){
    ...
    $('.note-meta button',root).onclick=()=>handleNoteAction('move');$$('[data-editor-add]',root).forEach(button=>button.onclick=()=>add(button.dataset.editorAdd));
