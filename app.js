(() => {
  ...
  function bindUnifiedEditor(root,record){
    ...
    $('[data-editor-color]',root).onpointerdown=event=>{event.preventDefault();const blockId=currentContent()?.closest('.content-block')?.dataset.blockId||record.blocks.at(-1)?.id;openTextColorPalette(event.currentTarget,blockId,null,null,record.id);};
