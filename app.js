(() => {
  ...
  function bindTypingHistory(editor){
    if(!editor)return;let groupOpen=false,groupTimer=null;
    editor.addEventListener('beforeinput',event=>{if(!editor.isConnected||state.readingMode||event.inputType==='historyUndo'||event.inputType==='historyRedo')return;if(!groupOpen){checkpoint();groupOpen=true;}clearTimeout(groupTimer);groupTimer=setTimeout(()=>groupOpen=false,1200);});
    editor.addEventListener('blur',()=>{clearTimeout(groupTimer);groupOpen=false;});
  }
  ...
  function renderIdeas() {
    ...
    $('#ideaLayer').innerHTML = visible.map(idea => {const childCount=note.ideas.filter(item=>item.parentId===idea.id).length;return `<div class="idea-node ${idea.root?'root':''} ${idea.formula?'formula-node':''} ${idea.collapsed?'collapsed':''} ${state.selectedObject?.type==='idea'&&state.selectedObject.id===idea.id?'selected':''}" data-idea-id="${idea.id}" style="left:${idea.x}px;top:${idea.y}px;width:${idea.width||220}px">${idea.formula?formulaToMathML(idea.formula):`<div class="idea-text">${escapeHtml(idea.text)}</div><textarea aria-label="Idea" readonly>${escapeHtml(idea.text)}</textarea>`}<span class="node-tools">${childCount?`<button data-toggle-branch="${idea.id}" aria-label="${escapeHtml(idea.collapsed?t('expand'):t('collapse'))}">${idea.collapsed?childCount:'−'}</button>`:''}<button data-add-child="${idea.id}" aria-label="${escapeHtml(t('addChild'))}">+</button><button data-delete-idea="${idea.id}" aria-label="${escapeHtml(t('delete'))}">×</button></span><span class="idea-resize" data-idea-resize aria-label="Resize idea"></span></div>`;}).join('');
    $$('.idea-node').forEach(element => {
      const idea = note.ideas.find(item => item.id === element.dataset.ideaId);
      $('textarea',element)?.addEventListener('input', event => { idea.text = event.target.value;$('.idea-text',element).textContent=idea.text;markChanged(); });
      $('textarea',element)?.addEventListener('keydown',event=>{if(event.key==='Tab'){event.preventDefault();addChildIdea(idea.id);}else if(event.key==='Enter'&&event.ctrlKey){event.preventDefault();idea.root?addChildIdea(idea.id):addSiblingIdea(idea.id);}else if(event.key==='ArrowLeft'&&idea.parentId&&event.altKey){event.preventDefault();focusIdea(idea.parentId);}else if(event.key==='ArrowRight'&&event.altKey){const child=note.ideas.find(item=>item.parentId===idea.id);if(child){event.preventDefault();focusIdea(child.id);}}else if((event.key==='ArrowUp'||event.key==='ArrowDown')&&idea.parentId&&event.altKey){const siblings=note.ideas.filter(item=>item.parentId===idea.parentId),index=siblings.indexOf(idea),next=siblings[index+(event.key==='ArrowUp'?-1:1)];if(next){event.preventDefault();focusIdea(next.id);}}else if(event.key==='Escape')event.target.blur();});
  ...
  function addIdea(parentId=null) {
    ...
    const root=note.ideas.find(item=>item.root),direction=parent&&root&&parent.x<root.x?-1:1,first=note.ideas.length===0,spawn=first?firstMindMapPosition(note):{x:center.x-70,y:center.y-24};note.ideas.push({id:id(),text:t('addIdea'),x:parent?parent.x+direction*260:spawn.x,y:parent?parent.y+siblings*76:spawn.y,width:220,root:first,parentId:parent?.id||null,collapsed:false,formula:null});
  ...
  function editIdea(ideaId){if(state.readingMode)return;const element=$(`[data-idea-id="${ideaId}"]`),input=$('textarea',element);if(!input)return;element.classList.add('editing');input.readOnly=false;input.focus();input.select();input.addEventListener('blur',()=>{input.readOnly=true;element.classList.remove('editing');},{once:true});}
  function focusIdea(ideaId){const element=$(`[data-idea-id="${ideaId}"]`);if(!element)return;selectObject('idea',ideaId,element);const input=$('textarea',element);input?.focus();input?.select();}
  function addFormulaIdea(latex){const note=activeNote();if(!note)return;const parent=note.ideas.find(item=>item.id===(state.selectedObject?.type==='idea'?state.selectedObject.id:null))||note.ideas.find(item=>item.root),root=note.ideas.find(item=>item.root),direction=parent&&root&&parent.x<root.x?-1:1,center=viewportCenterInWorld(),siblings=parent?note.ideas.filter(item=>item.parentId===parent.id).length:0;note.ideas.push({id:id(),text:'',formula:latex,x:parent?parent.x+direction*260:center.x-90,y:parent?parent.y+siblings*76:center.y-25,width:240,root:note.ideas.length===0,parentId:parent?.id||null,collapsed:false});}
  ...
  function autoLayoutMindMap(){
    ...
    const estimatedHeight=idea=>Math.max(52,Math.ceil((String(idea.text||'').length*7)/Math.max(110,(idea.width||220)-32))*20+28),gap=70;
    const layoutSide=(branches,direction)=>{
      let cursor=0;const placed=[];
      const place=(idea,parent)=>{if(visited.has(idea.id))return;visited.add(idea.id);placed.push(idea);const children=note.ideas.filter(item=>item.parentId===idea.id&&!visited.has(item.id));if(children.length){children.forEach(child=>place(child,idea));idea.y=children.reduce((sum,child)=>sum+child.y,0)/children.length;}else{idea.y=cursor;cursor+=estimatedHeight(idea)+34;}idea.x=direction>0?parent.x+(parent.width||220)+gap:parent.x-(idea.width||220)-gap;};
      branches.forEach(branch=>place(branch,root));
  ...
  function unifiedEditorToolbar(){return `<button data-editor-format="bold"><strong>B</strong></button><button data-editor-format="italic"><i>I</i></button><button data-editor-format="underline"><u>U</u></button><button data-editor-format="strikeThrough"><s>S</s></button><span></span><button data-editor-add="text">T</button><button data-editor-add="heading1">H1</button><button data-editor-add="heading">H2</button><button data-editor-add="heading3">H3</button><button data-editor-add="todo">☐</button><button data-editor-add="bullets">•</button><button data-editor-add="quote">❝</button><button data-editor-add="divider">—</button><button data-editor-image title="${escapeHtml(t('image'))}"><svg><use href="#i-image"/></svg></button><button data-editor-link>${escapeHtml(t('link'))}</button><button data-editor-formula>ƒx</button>`;}
  function unifiedEditorBlockMarkup(block,only=false){
    ...
    if(block.type==='divider')return `<div class="content-block secondary-block" ${attrs} contenteditable="false"><hr class="block-divider"></div>`;
    if(block.type==='formula')return `<div class="content-block secondary-block" ${attrs} contenteditable="false"><button class="formula-block" data-editor-formula-block="${block.id}">${formulaToMathML(block.latex||'')}</button></div>`;
    if(block.type==='image')return `<div class="content-block secondary-block" ${attrs} contenteditable="false"><button class="image-block" data-editor-image-block="${block.id}"><img src="${escapeHtml(block.src||'')}" alt="${escapeHtml(block.alt||'')}"></button></div>`;
    const marker=block.type==='todo'?'<button class="todo-check" tabindex="-1"></button>':block.type==='bullets'?'<span class="bullet-marker">•</span>':'';
    return `<div class="content-block secondary-block ${block.done?'done':''}" ${attrs} style="--block-color:${block.color||'inherit'}">${marker}<div class="block-content secondary-block-content" style="color:var(--block-color)" data-placeholder="${only&&!block.text&&!block.html?escapeHtml(t('emptyNote')):''}">${renderBlockHtml(block)}</div></div>`;
  }
  function openUnifiedSlashMenu(anchor,record,blockId){
    const menu=$('#slashMenu'),items=['text','heading1','heading','heading3','todo','bullets','quote','formula','divider'];menu.innerHTML=items.map(type=>`<button data-type="${type}" role="option">${escapeHtml(t(type))}</button>`).join('');menu.dataset.activeIndex='-1';menu.setAttribute('role','listbox');const rect=anchor.getBoundingClientRect();menu.style.left=`${Math.min(rect.left+20,innerWidth-210)}px`;menu.style.top=`${Math.min(rect.bottom+3,innerHeight-230)}px`;menu.classList.add('open');
  ...
  function openUnifiedBlockContextMenu(record,blockId,x,y){const menu=$('#contextMenu'),block=record.blocks.find(item=>item.id===blockId);if(!block)return;menu.innerHTML=`<button data-editor-context="duplicate">${escapeHtml(t('duplicate'))}</button><button data-editor-context="formula">${escapeHtml(t('formula'))}</button><button data-editor-context="delete" class="danger">${escapeHtml(t('delete'))}</button>`;openMenu(menu,x,y);$$('[data-editor-context]',menu).forEach(button=>button.onclick=()=>{const action=button.dataset.editorContext;closeMenus();if(action==='duplicate'){checkpoint();record.blocks.splice(record.blocks.indexOf(block)+1,0,{...clone(block),id:id()});renderUnifiedEditors();markChanged();}else if(action==='formula')openFormulaDialog(record.primary?'document':'editor',blockId,record.id);else if(action==='delete'){checkpoint();record.blocks=record.blocks.length===1?[createEditorBlock('text')]:record.blocks.filter(item=>item.id!==blockId);if(record.primary)activeNote().blocks=record.blocks;else activeNote().editors.find(item=>item.id===record.id).blocks=record.blocks;renderUnifiedEditors();markChanged();}});}
  function renderUnifiedEditors(focus=null){
    const note=activeNote();if(!note)return;
    const editorScroll=new Map($$('.unified-editor-card',$('#canvasWorld')).map(root=>[root.dataset.editorId||'primary',$('.block-list',root)?.scrollTop||0]));
    $$('.document-card',$('#canvasWorld')).forEach(element=>element.remove());
  ...
  function bindUnifiedEditor(root,record){
    ...
    $$('[data-editor-format]',root).forEach(button=>button.onpointerdown=event=>{event.preventDefault();const content=currentContent();if(!content)return;document.execCommand('styleWithCSS',false,false);document.execCommand(button.dataset.editorFormat,false,null);syncDocument();markChanged();});
    $('[data-editor-link]',root).onpointerdown=event=>{event.preventDefault();const content=currentContent();if(!content)return;let url=prompt('https://');if(!url)return;if(!/^[a-z][a-z0-9+.-]*:/i.test(url))url='https://'+url;if(!/^(https?:|mailto:)/i.test(url)){toast('Only web and email links are supported.');return;}const selection=getSelection();if(selection&&!selection.isCollapsed)document.execCommand('createLink',false,url);else document.execCommand('insertHTML',false,`<a href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(url)}</a>`);syncDocument();markChanged();};
    $('[data-editor-formula]',root).onpointerdown=event=>{event.preventDefault();const blockId=currentContent()?.closest('.content-block')?.dataset.blockId||null;openFormulaDialog(record.primary?'document':'editor',blockId,record.id);};
    $('[data-editor-image]',root).onclick=()=>{const afterId=currentContent()?.closest('.content-block')?.dataset.blockId||null,input=document.createElement('input');input.type='file';input.accept='image/*';input.onchange=()=>prepareNoteImage(input.files[0],properties=>add('image',afterId,properties));input.click();};
    bindTypingHistory(list);
    // The editor is a single, continuous contenteditable surface. Intercept at
  ...
  function bindWorldDrag(element, object, onMove = () => {}, options = {}) {
    element.addEventListener('pointerdown', event => {
      if (state.readingMode||state.tool !== 'select' || event.target.closest('button,[contenteditable],[data-secondary-resize],[data-idea-resize],.document-resize-handle') || (event.target.closest('input,textarea')&&!event.target.readOnly)) return;
  ...
  function applyReadingMode(){const pane=$('.work-pane'),button=$('#readingModeButton');pane.classList.toggle('reading-mode',state.readingMode);button?.setAttribute('aria-pressed',String(state.readingMode));if(button)button.title=t(state.readingMode?'editingMode':'readingMode');$$('.unified-editor-title').forEach(input=>input.readOnly=state.readingMode);$$('.block-list').forEach(editor=>editor.contentEditable=String(!state.readingMode));if(state.readingMode){setTool('pan');clearSelection();}}
  ...
  function closeMenus(){ $$('.menu-surface.open,.slash-menu.open,.fluent-options.open').forEach(menu=>menu.classList.remove('open'));$$('.fluent-select').forEach(button=>button.setAttribute('aria-expanded','false')); }
  ...
  function openObjectContextMenu(type,objectId,x,y){
    ...
    else if(type==='block')actions=[['duplicate',t('duplicate')],['formula',t('formula')],['delete',t('delete')]];
  ...
  function printInkDataUrl(note){
    const canvas=document.createElement('canvas');canvas.width=4000;canvas.height=3000;
    const context=canvas.getContext('2d',{alpha:true}),vectors=note.inkStrokes||[];
    if(vectors.length)vectors.forEach(stroke=>drawStroke(context,stroke,1,false));
    else if(note.ink){
      const fallback=$('#inkCanvas');
      if(fallback?.width){
        context.drawImage(fallback,0,0,4000,3000);
        if(document.body.classList.contains('theme-dark')){
          const pixels=context.getImageData(0,0,4000,3000),data=pixels.data;
          for(let index=0;index<data.length;index+=4){
            if(data[index+3]<4)continue;
            const high=Math.max(data[index],data[index+1],data[index+2]),low=Math.min(data[index],data[index+1],data[index+2]);
            if(high>205&&high-low<34)data[index]=data[index+1]=data[index+2]=23;
          }
  ...
  function normalizePrintTextColors(root){
    root.querySelectorAll('[style]').forEach(element=>{
      const inline=element.style.color.trim().toLowerCase(),blockColor=element.style.getPropertyValue('--block-color').trim().toLowerCase();
      if(inline==='#fff'||inline==='#ffffff'||inline==='white'||inline==='rgb(255, 255, 255)')element.style.color='#171717';
      if(blockColor==='#fff'||blockColor==='#ffffff'||blockColor==='white'||blockColor==='rgb(255, 255, 255)')element.style.setProperty('--block-color','#171717');
    });
  }
  function preparePrintSheet(){
    ...
    const ideas=$('#ideaLayer').cloneNode(true);ideas.removeAttribute('id');ideas.querySelectorAll('[id]').forEach(element=>element.removeAttribute('id'));ideas.querySelectorAll('.node-tools,.idea-resize,textarea').forEach(element=>element.remove());scene.append(ideas);
    ...
    scene.querySelectorAll('.selected,.dragging,.group-dragging').forEach(element=>element.classList.remove('selected','dragging','group-dragging'));
    normalizePrintTextColors(scene);board.append(scene);sheet.append(heading,board);
  }
  ...
  const compactPortraitQuery='(max-width: 1100px) and (orientation: portrait)';
  const isPortraitMobile=()=>matchMedia(compactPortraitQuery).matches;
  ...
  function bindEvents() {
    ...
    document.addEventListener('click',event=>{const inside=event.composedPath().some(node=>node?.matches?.('.menu-surface,.fluent-picker,#noteMenuButton,#workspacePicker,#workspaceMore,#sortNotesButton'));if(!inside)closeMenus();});
    ...
    matchMedia(compactPortraitQuery).addEventListener('change',event=>{state.mobileStage=event.matches?(activeNote()?'editor':'notes'):'editor';applyPaneLayout();syncMobileHierarchy();});
  }
