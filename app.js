(() => {
  ...
  function bindUnifiedEditor(root,record){
    ...
    $$('[data-editor-format]',root).forEach(button=>button.onpointerdown=event=>{event.preventDefault();const content=currentContent();if(!content)return;document.execCommand(button.dataset.editorFormat,false,null);syncDocument();markChanged();});
