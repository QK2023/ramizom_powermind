(() => {
  ...
  function bindUnifiedEditor(root,record){
    ...
    const list=$('.block-list',root),currentContent=()=>activeEditorContent(root),syncDocument=()=>{const elements=$$('.content-block',list),next=[];elements.forEach(blockElement=>{const block=record.blocks.find(item=>item.id===blockElement.dataset.blockId),content=$('.block-content',blockElement);if(block&&content){block.text=content.textContent;block.html=sanitizeRichHtml(content.innerHTML);next.push(block);}});if(next.length){if(next.length!==record.blocks.length)record.blocks.splice(0,record.blocks.length,...next);return true;}const replacement=createEditorBlock('text',{text:list.textContent||'',html:escapeHtml(list.textContent||'')});record.blocks.splice(0,record.blocks.length,replacement);queueMicrotask(()=>rerender(replacement.id));return false;};
    ...
    bindTypingHistory(list);list.addEventListener('input',()=>{if(state.readingMode)return;syncDocument();const content=currentContent(),blockElement=content?.closest('.content-block'),block=record.blocks.find(item=>item.id===blockElement?.dataset.blockId);if(block?.text==='/')openUnifiedSlashMenu(blockElement,record,block.id);markChanged();});list.addEventListener('keydown',event=>{const content=currentContent(),blockElement=content?.closest('.content-block');if(!blockElement)return;const forwarded=new Proxy(event,{get:(target,key)=>key==='currentTarget'?content:typeof target[key]==='function'?target[key].bind(target):target[key]});handleUnifiedBlockKeydown(forwarded,record,blockElement.dataset.blockId,blockElement);});
