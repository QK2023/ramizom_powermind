(() => {
  ...
  function bindTypingHistory(editor){
    ...
    editor.addEventListener('beforeinput',event=>{if(state.readingMode||event.inputType==='historyUndo'||event.inputType==='historyRedo')return;if(!groupOpen){checkpoint();groupOpen=true;}clearTimeout(groupTimer);groupTimer=setTimeout(()=>groupOpen=false,1200);});
