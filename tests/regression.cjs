...
async function run() {
  ...
  const bootTheme = html.indexOf("body.classList.add(dark?'theme-dark':'theme-light')");
  assert(bootTheme > 0, 'The saved theme is applied before the app boots');
  assert(bootTheme < html.indexOf('class="startup-splash"'), 'The saved theme is applied before the splash markup, so the first paint is already themed');
  assert(html.indexOf("themeMeta.setAttribute('content',color)") > 0 && html.indexOf("themeMeta.setAttribute('content',color)") < html.indexOf('class="startup-splash"'), 'The theme colour used by the window and status bar is corrected before the first paint');
